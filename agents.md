...
- **CABQ Comprehensive Plan Action Application** — internal React + TypeScript + Vite app for documenting departmental actions against the ABC Comprehensive Plan hierarchy, with cascading selects, library, export, print, and **Admin Console**. See [`README.md`](README.md).
...
- **Multi-page build:** `index.html` (user-facing app) + `admin.html` (admin console). Both share `src/` utilities and localStorage.
- **Admin Console (`src/admin/`):** Submissions list with full-text search, detail/edit page, print, share. Placeholder auth via `src/auth.ts` / `src/useAuth.ts` with `comp-plan-admin` role — SSO integration pending. Test seed data in `src/admin/seedTestData.ts` (remove before production).
